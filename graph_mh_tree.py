...
def random_tree(n, k):
    ...
    return [(p, minhash_sig(S, k), simhash_sig(S, k), len(S) == m) for p, S in G]

def compute_error(G, k, delta1, delta2):
    N = len(G)
    FN1, FN2, TN = 0, 0, 0
    FP1, FP2, TP = 0, 0, 0
...
def test_size(k):
    ...
    for test in range(100):
        for delta in range(1, X+1):
            ...
            G = random_tree(N, k)
            FP1, FN1, FP2, FN2 = compute_error(G, k, 0.416666667, 0.671173911)
            RP1[delta].append(FP1)
            RN1[delta].append(FN1)
            RP2[delta].append(FP2)
            RN2[delta].append(FN2)
